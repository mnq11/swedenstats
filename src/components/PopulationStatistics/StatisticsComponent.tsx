    ...
    if (active && payload && payload.length) {
        return (
            <div style={{
                ...
                <p style={{fontWeight: 'bold'}}>{`Age : ${payload[0].payload.name}`}</p>
                <hr style={{borderTop: '1px solid #fff'}}/>
                <p>{`Population : ${payload[0].value}`}</p>
                <p><FontAwesomeIcon icon={faMale} /> {` : ${payload[0].payload.Male.Single + payload[0].payload.Male.Married + payload[0].payload.Male.Widowed + payload[0].payload.Male.Divorced}`}</p>
                <p><FontAwesomeIcon icon={faFemale} /> {` : ${payload[0].payload.Female.Single + payload[0].payload.Female.Married + payload[0].payload.Female.Widowed + payload[0].payload.Female.Divorced}`}</p>
                <hr style={{borderTop: '1px solid #fff'}}/>
                <p><FontAwesomeIcon icon={faMale} /> Single: {` ${payload[0].payload.Male.Single}`}</p>
                <p><FontAwesomeIcon icon={faMale} /> <FontAwesomeIcon icon={faRing} /> {` Married: ${payload[0].payload.Male.Married}`}</p>
                <p><FontAwesomeIcon icon={faMale} /> <FontAwesomeIcon icon={faHeartBroken} /> {` Widowed: ${payload[0].payload.Male.Widowed}`}</p>
                <p><FontAwesomeIcon icon={faMale} /> <FontAwesomeIcon icon={faWindowClose} /> {` Divorced: ${payload[0].payload.Male.Divorced}`}</p>
                <hr style={{borderTop: '1px solid #fff'}}/>
                <p><FontAwesomeIcon icon={faFemale} /> Single: {` ${payload[0].payload.Female.Single}`}</p>
                <p><FontAwesomeIcon icon={faFemale} /> <FontAwesomeIcon icon={faRing} /> {` Married: ${payload[0].payload.Female.Married}`}</p>
                <p><FontAwesomeIcon icon={faFemale} /> <FontAwesomeIcon icon={faHeartBroken} /> {` Widowed: ${payload[0].payload.Female.Widowed}`}</p>
                <p><FontAwesomeIcon icon={faFemale} /> <FontAwesomeIcon icon={faWindowClose} /> {` Divorced: ${payload[0].payload.Female.Divorced}`}</p>
            </div>
        );
    }
    return null;
...
const StatisticsComponent: React.FC<Props> = ({statsData}) => {
    ...
    const yearPopulationData = chartData.reduce<{ [key: string]: { name: string, value: number } }>((accumulator, current) => {
    ...
    const maritalStatusPopulationData = chartData.reduce<{ [key: string]: { name: string, value: number } }>((accumulator, current) => {
    ...
    const agePopulationData = filteredData.reduce<{ [key: string]: { name: string, value: number, Male: {Single: number, Married: number, Widowed: number, Divorced: number}, Female: {Single: number, Married: number, Widowed: number, Divorced: number} } }>((accumulator, current) => {
    ...
    const genderPopulationData = filteredData.reduce<{ [key: string]: { name: string, value: number } }>((accumulator, current) => {
    ...
    const agePopulationChartData = Object.values(agePopulationData).map(obj => ({
        ...obj,
        name: parseInt(obj.name)
    })).sort((a, b) => a.name - b.name);    const genderPopulationChartData = Object.values(genderPopulationData);
        ...
        <div className="StatisticsComponent">
            ...
            <label>Select Marital Status:
                <select value={selectedMaritalStatus} onChange={e => setSelectedMaritalStatus(e.target.value)}>
                    ...
                    {Object.keys(maritalStatusPopulationChartData).map(status => <option key={status}>{status}</option>)}
                </select>
            </label>
            <div className="chart-container">
                <div className="chart">
                    <h2>Population by Age in {selectedYear}</h2>
                    <BarChart width={600} height={300} data={agePopulationChartData} style={{ backgroundColor: darkMode.backgroundColor }}>
                ...
                <div className="chart">
                    ...
                    <PieChart width={600} height={300} style={{ margin: 'auto' }}>
                        <Pie
                            ...
                            label={({ name, percent }) => `${name}: ${(percent * 100).toFixed(0)}%`} // show percentage in labels
                        >
                            {
                ...
                <div className="chart">
                    ...
                    <BarChart width={600} height={300} data={yearPopulationChartData}>
                        ...
                        <Bar dataKey="value" barSize={20} fill="#413ea0">
                            {
                                yearPopulationChartData.map((entry, index) => <Cell key={`cell-${index}`} fill={COLORS[index % COLORS.length]} />)
                            }
                        </Bar>
                    </BarChart>
